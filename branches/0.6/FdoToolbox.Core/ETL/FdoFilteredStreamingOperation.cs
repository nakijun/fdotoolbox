...
namespace FdoToolbox.Core.ETL
{
    ...
    /// 
    /// This operation supports filtering (discarding) of certain features
    /// </summary>
    public abstract class FdoFilteredStreamingOperation : IFdoOperation
    {
        public IEnumerable<FdoFeature> Process(IEnumerable<FdoFeature> features)
        {
            foreach (FdoFeature feat in features)
            {
                if (ProcessFeature(feat))
                    yield return feat;
            }
        }
        ...
        /// <returns>true to send this feature down the pipeline, false to discard it</returns>
        protected abstract bool ProcessFeature(FdoFeature feat);
    }
}
